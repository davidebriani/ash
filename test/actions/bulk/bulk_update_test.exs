defmodule Ash.Test.Actions.BulkUpdateTest do
  ...
  defmodule MultitenantTag do
    ...
    identities do
      identity :name, [:name], pre_check_with: MultitenantTag
    end
  end
  ...
  describe "load" do
    ...
    test "allows loading paginated many_to_many relationship after changing it" do
      related_post1 = Ash.create!(Post, %{title: "Related 1"})
      related_post2 = Ash.create!(Post, %{title: "Related 2"})
      ...
      assert %Ash.BulkResult{records: [post]} =
               Ash.bulk_update!(
                 [post],
                 :add_related_posts,
                 %{related_post_ids: [related_post1.id]},
                 resource: Post,
                 strategy: :stream,
                 return_records?: true,
                 return_errors?: true,
      ...
      assert %Ash.BulkResult{records: [post]} =
               Ash.bulk_update!(
                 [post],
                 :add_related_posts,
                 %{related_post_ids: [related_post2.id]},
                 resource: Post,
                 strategy: :stream,
                 return_records?: true,
                 return_errors?: true,
    ...
    test "allows loading paginated many_to_many relationship for multitenant resources" do
      ...
      existing_tag = Ash.create!(MultitenantTag, %{name: "existing"}, tenant: tenant)
